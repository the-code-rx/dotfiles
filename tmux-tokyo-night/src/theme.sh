...
if [ "$theme_disable_plugins" -ne 1 ]; then
  ...
  for plugin in "${plugins[@]}"; do
    ...
    if [ ! -f "${CURRENT_DIR}/plugin/${plugin}.sh" ]; then
      ...
      plugin_output="#[fg=${PALLETE[bg_highlight]},bg=${PALLETE[$accent_color]}]$(load_plugin)#[none]"
      plugin_output_string=""

      plugin_icon_output="${separator_icon_start}#[fg=${PALLETE[bg_dark]},bg=${PALLETE[$accent_color_icon]}]${plugin_icon}${separator_icon_end}"
